 ...
 *  优先级: props、methods、data、computed 对象中的属性不能出现重复,优先级和列出顺序一直
...
export function initState(vm: Component) {
  ...
  /**
   * 其实这里也能看出,computed 和 watch 在本质上没有区别,都是通过 watcher 去实现的响应式
   * 非要说有区别,那也只能是使用方式上的区别,简单来说:
   *  1.watch: 适用于当数据变化是执行异步或者开销较大的操作时使用,即需要长时间等待的操作可以放在 watch 中
   *  2.computed:其中可以使用异步方法,但没有任何意义,所以 computed 更适合做一些同步计算
   */
}
 ...
 *     methods 中的 key 与 Vue 实例上已有的方法重叠,一般是一些内置的方法,比如以 $ 和 _ 开头的方法
